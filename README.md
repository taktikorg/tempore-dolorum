**@taktikorg/tempore-dolorum**

The most lightweight, customizable React markdown component.

[![npm version](https://badge.fury.io/js/@taktikorg/tempore-dolorum.svg)](https://badge.fury.io/js/@taktikorg/tempore-dolorum) [![downloads](https://badgen.net/npm/dy/@taktikorg/tempore-dolorum)](https://npm-stat.com/charts.html?package=@taktikorg/tempore-dolorum)
...
`@taktikorg/tempore-dolorum` uses a heavily-modified fork of [simple-markdown](https://github.com/Khan/simple-markdown) as its parsing engine and extends it in a number of ways to make your life easier. Notably, this package offers the following additional benefits:
...
Install `@taktikorg/tempore-dolorum` with your favorite package manager.

```shell
npm i @taktikorg/tempore-dolorum
```

## Usage

`@taktikorg/tempore-dolorum` exports a React component by default for easy JSX composition:

ES6-style usage\*:

```jsx
import Markdown from '@taktikorg/tempore-dolorum'
...
Any conflicts between passed `props` and the specific properties above will be resolved in favor of `@taktikorg/tempore-dolorum`'s code.
...
One of the most interesting use cases enabled by the HTML syntax processing in `@taktikorg/tempore-dolorum` is the ability to use any kind of element, even ones that aren't real HTML tags like React component classes.
...
`@taktikorg/tempore-dolorum` also handles JSX interpolation syntax, but in a minimal way to not introduce a potential attack vector. Interpolations are sent to the component as their raw string, which the consumer can then `eval()` or process as desired to their security needs.
...
````tsx
import Markdown, { RuleType } from '@taktikorg/tempore-dolorum'
...
To get around this, `@taktikorg/tempore-dolorum` left-trims approximately as much whitespace as the first line inside the HTML block. So for example:
...
If desired, the compiler function is a "named" export on the `@taktikorg/tempore-dolorum` module:

```jsx
import { compiler } from '@taktikorg/tempore-dolorum'
import React from 'react'
import { render } from 'react-dom'
...
See [Github Releases](https://github.com/probablyup/@taktikorg/tempore-dolorum/releases).
